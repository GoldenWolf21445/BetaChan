package de.senseless.betachan.handler;

import de.senseless.betachan.enums.ToolType;
import de.senseless.betachan.item.*;

public class ItemHandler {

    public static final Item BASILISKSCALE = new Item("Basilisk Scale");
    public static final Item BEASTFUR = new Item("Beast Fur");
    public static final Item BONE = new Item("Bone");
    public static final Item COAL = new Item("Coal");
    public static final Item CRYSTAL = new Item("Crystal");
    public static final Item DRAGONHEART = new Item("Dragon Heart");
    public static final Item DRAGONSCALE = new Item("Dragon Scale");
    public static final Item DRAGONTOOTH = new Item("Dragon Tooth");
    public static final Item GRIFFINCLAW = new Item("Griffin Claw");
    public static final Item GRIFFINFEATHER = new Item("Griffin Feather");
    public static final Item IRONINGOT = new Item("Iron Ingot",true);
    public static final Item KITSUNETAIL = new Item("Tail of a Kitsune");
    public static final Item MEAT = new Item("Meat");
    public static final Item COOKEDMEAT = new Item("Cooked Meat",true);
    public static final Item RAWIRON = new Item("Raw Iron");
    public static final Item SPIDERSTRING = new Item("String");
    public static final Item STONE = new Item("Stone");
    public static final Item WOLFFUR = new Item("Wolf Fur");
    public static final Item LIZARDSCALE = new Item("Lizard Scale");
    public static final Item ANGELWING = new Item("Angel Wing");
    public static final Item MAGICLEAF = new Item("Magic Leaf");
    public static final Item UNICORNHORN = new Item("Unicorn Horn");
    public static final Item FAIRYWING = new Item("Fairy Wing");
    public static final Item POTION = new Item("Potion",true);
    public static final Item WOOD = new Item("Wood");
    public static final Item WOODENAXE = new Item("Wooden Axe", true, "Wood|4", 100, ToolType.AXE);
    public static final Item WOODENPICKAXE = new Item("Wooden Pickaxe", true, "Wood|3---Stick|2", 100, ToolType.PICKAXE);
    public static final Item BASKET = new Item("Basket", false, "", 100, ToolType.BASKET);
    public static final Item STICK = new Item("Stick",true,"Wood|1");
    public static final Item APPLE = new Item("Apple");

}
